...
classBody 
    :   '{' {klass.setCurrentBlock(klass.getTopBlock());}
        (classBodyDeclaration
        )* 
...
classBodyDeclaration 
    :   ';'
    |   'static' {klass.newBlock(RBlock.STATIC_INITIALIZER);} 
        block
        {klass.backToUpperBlock();}
...
variableDeclarator returns [RIdentifier id]
    :   IDENTIFIER {
          $id = new RIdentifier(); 
...
localVariableDeclaration 
    :   variableModifiers type {type = $type.text;}
        variableDeclarator
        (',' variableDeclarator
        )*
    ;
