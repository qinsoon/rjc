...
by David Callaham et al. \rjc only considers numeric constants (including integer,
floating point number, byte and character. Address-alike constants are currently
ignored. 

This optimization pass does two passes over the code to collect informatin, 
and performs the propagation in the end. 
It uses results from \emph{Class Hierarchy Analysis}, 
\emph{Points-to Analysis} and \emph{Call Graph} to resolve cases such
that the implementation of abstract methods may return constants. Note
this optimization does not directly change AST, but store constant information
internally and provide a query method for code generation. 
